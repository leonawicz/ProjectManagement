...
For example, the \texttt{projman.R} script contains all the \texttt{projman} functions. 
Although this script has a complete, corresponding \texttt{projman.Rmd} file, I decided to subsequently break out the functions and other content from \texttt{projman.Rmd} into multiple Rmd files.
...
\hlstd{c1a} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{"_output.yaml"}\hlstd{,} \hlstr{"navbar.html"}\hlstd{)}
...
\hlstd{from} \hlkwb{<-} \hlkwd{c}\hlstd{(}
        \hlstd{c0,}     \hlkwd{rep}\hlstd{(}\hlstr{"projman.Rmd"}\hlstd{,} \hlkwd{length}\hlstd{(c2b)), c2, c2,} \hlkwd{rep}\hlstd{(}\hlstr{"example.R"}\hlstd{,} \hlkwd{length}\hlstd{(c1a)), c1b,} \hlkwd{rep}\hlstd{(}\hlkwd{c}\hlstd{(c1a, c1c),} \hlkwc{each}\hlstd{=}\hlkwd{length}\hlstd{(c4))}

\hlstd{)}
\hlstd{to} \hlkwb{<-} \hlkwd{c}\hlstd{(}
        \hlstd{c2a, c2b, c3, c4, c1a,} \hlstr{"example.R"}\hlstd{,} \hlkwd{rep}\hlstd{(c4,} \hlkwd{length}\hlstd{(}\hlkwd{c}\hlstd{(c1a, c1c)))}
\hlstd{)}
\end{alltt}
\end{kframe}
\end{knitrout}
